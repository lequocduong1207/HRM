...
export default function UserManagement() {
  ...
  const { roles, loading: rolesLoading } = useRoles(false, true);
  ...
  const getRoleBadge = (role: string) => {
    ...
    switch (role) {
      case "admin":
        return <Badge color="error">{displayName}</Badge>;
      case "hr_manager":
        return <Badge color="warning">{displayName}</Badge>;
      case "department_manager":
        return <Badge color="dark">{displayName}</Badge>;
      case "employee":
        return <Badge color="info">{displayName}</Badge>;
      default:
        return <Badge color="light">{displayName}</Badge>;
    }
  };
